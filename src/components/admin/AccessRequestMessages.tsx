...
import { Send, Paperclip, Image, Download, MessageCircle, X } from 'lucide-react';
...
interface AccessRequestMessagesProps {
  requestId: string;
  userEmail: string;
  onClose: () => void;
}

export const AccessRequestMessages: React.FC<AccessRequestMessagesProps> = ({
  requestId,
  userEmail,
  onClose
}) => {
  ...
  return (
    <Card className="border-2 border-black max-w-2xl mx-auto">
      <CardHeader className="flex flex-row items-center justify-between">
        <div>
          <CardTitle className="flex items-center gap-2">
            <MessageCircle className="w-5 h-5" />
            Messages with {userEmail}
          </CardTitle>
        </div>
        <Button
          onClick={onClose}
      ...
      <CardContent>
        {/* Messages */}
        <div className="h-96 overflow-y-auto mb-4 space-y-3 border border-gray-200 rounded p-3">
          {messages.length === 0 ? (
